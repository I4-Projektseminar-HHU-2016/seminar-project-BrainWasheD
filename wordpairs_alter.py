...
class word_pairsAlter():
    ...
    def pairings(self, given_words, texts):
        ...
        for freq in given_words:
            (self.key, self.val) = freq
            self.keys.append(self.key)
            self.keysHT.append('#'+self.key)
            ...
            for text in texts.values():
                if (str(word[0]) in text) or ('#'+str(word[0]) in text):
                    for elem in text:
                        ...
                        if (self.word != str(word[0])) and (self.word != ('#'+str(word[0])) and (self.word not in self.keys) and (self.word not in self.keysHT)):
